    ...
    for meta in metas:
        if meta == "doubt":
            ...
            try:
                ...
                mapie.fit(X_tr, y_tr)
                y_hat = mapie.predict(X_te)
                for coverage in coverages:
                    if coverage != 1:
                        tmp = get_metrics_test(y_te, y_hat, np.zeros(y_te.shape))
                    else:
                        tmp = get_metrics_test(y_te, y_hat, np.ones(y_te.shape))
                    tmp["target_coverage"] = coverage
                    res = pd.concat([res, tmp], axis=0)
...
def fetch_data_ML(dataset, dataID_dict=dataID_dict):
    import sklearn.datasets as skd
    X, y = skd.fetch_openml(data_id=dataID_dict[dataset], return_X_y=True)
    if dataset == "sulfur":
        X = X.drop("y2", axis=1)
    elif dataset in ["seattle", "abalone", "mercedes", "soil", "diamonds", "brazilian_houses", "bikes"]:
        # X = X.drop(["cut", "color", "clarity"])
        cat_atts = list(
...
def main(dataset, regressors, metas, nj=1, seed=42, nboots=None):
    ...
    for reg_string in regressors:
        ...
        if (X.shape[0] >= 1000) & (X.shape[0] < 100000):
...
if __name__ == "__main__":
    ...
    list_datasets = list(dataID_dict.keys())
