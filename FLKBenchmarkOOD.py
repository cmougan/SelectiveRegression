...
#
# def get_results_doubt(coverages, interval_val_new, interval_te_new, interval_val, interval_te, y_test, y_hat, y_train, test_dataset):
#     res = pd.DataFrame()
...
def get_results_doubt(coverages, dbt, X_val, X_test, y_test, y_train, nb, nj, uncertainty, test_dataset):
    if dbt._model.__class__.__name__ == "NeuralNetRegressor":
...
def get_results_plugin(coverages, rplug, error, X_test, X_val, y_test, y_hat, y_train, test_dataset):
    res = pd.DataFrame()
    for coverage in coverages:
...
def get_results_scross(coverages, rcross, X_test, X_val, y_test, y_hat, y_train, test_dataset):
    res = pd.DataFrame()
    for coverage in coverages:
...
def get_results_crossfit(coverages, rcross, X_test, X_val, y_test, y_hat, y_train, test_dataset):
    res = pd.DataFrame()
    for coverage in coverages:
        sel_te = rcross.select(X_test, X_val, coverage)
        if coverage != 1:
...
def get_results_mapie(coverages, mapie, X_test, X_val, y_test, y_train, uncertainty, test_dataset):
...
def experiment(
    ...
    scalerY.fit(y_tr.reshape(-1, 1))
    y_train = scalerY.transform(y_train.reshape(-1, 1)).flatten()
    y_val = scalerY.transform(y_val.reshape(-1, 1)).flatten()
    ...
    ood_sets = folk_dict[dataset]
    dict_ood_tests = get_testsets(dataset, seed, te, scaler, scalerY, perc_test, data_folder="data/folktables/")
    for meta in metas:
        if meta == "doubt":
            ...
            res = get_results_doubt(coverages, dbt, X_val, X_test, y_test, y_train, nb, nj, uncertainty, dataset)
            #over all ood datasets
            for key in ood_sets:
                X_ood = dict_ood_tests[key][0]
                y_ood = dict_ood_tests[key][1]
                # y_ood = y_ood.reshape(-1, 1)
                tmp = get_results_doubt(coverages, dbt, X_val, X_ood, y_ood, y_train, nb, nj, uncertainty, key)
                res = pd.concat([res, tmp], axis=0)
        elif meta == "plugin":
            ...
            res = get_results_plugin(coverages, rplug, error, X_test, X_val, y_test, y_hat, y_train, dataset)
            #over all ood datasets
            for key in ood_sets:
                X_ood = dict_ood_tests[key][0]
                y_ood = dict_ood_tests[key][1]
                # y_ood = y_ood.reshape(-1, 1)
                y_hat_ood = rplug.predict(X_ood)
                error_ood = (y_ood - y_hat_ood) ** 2
                tmp = get_results_plugin(coverages, rplug, error_ood, X_ood, X_val, y_ood, y_hat_ood, y_train, key)
                res = pd.concat([res, tmp], axis=0)
        elif meta == "scross":
            rcross = SCross(reg_base)
            rcross.fit(X_train, y_train)
            y_hat = rcross.predict(X_test)
            res =  get_results_scross(coverages, rcross, X_test, X_val, y_test, y_hat, y_train, dataset)
            #over all ood datasets
            for key in ood_sets:
                X_ood = dict_ood_tests[key][0]
                y_ood = dict_ood_tests[key][1]
                # y_ood = y_ood.reshape(-1, 1)
                y_hat_ood = rcross.predict(X_ood)
                tmp = get_results_scross(coverages, rcross, X_ood, X_val, y_ood, y_hat_ood, y_train, key)
                res = pd.concat([res, tmp], axis=0)
        elif meta == "crossfit":
            rcross = CrossFit(reg_base)
            rcross.fit(X_train, y_train)
            y_hat = rcross.predict(X_test)
            res = get_results_crossfit(coverages, rcross, X_test, X_val, y_test, y_hat, y_train, dataset)
            #over all ood datasets
            for key in ood_sets:
                X_ood = dict_ood_tests[key][0]
                y_ood = dict_ood_tests[key][1]
                # y_ood = y_ood.reshape(-1, 1)
                y_hat_ood = rcross.predict(X_ood)
                tmp = get_results_crossfit(coverages, rcross, X_ood, X_val, y_ood, y_hat_ood, y_train, key)
                res = pd.concat([res, tmp], axis=0)
        elif meta == "mapie":
            if reg_base.__class__.__name__ == "NeuralNetRegressor":
            ...
            res = get_results_mapie(coverages, mapie, X_test, X_val, y_test, y_train, uncertainty, dataset)
            #over all ood datasets
            for key in ood_sets:
                X_ood = dict_ood_tests[key][0]
                y_ood = dict_ood_tests[key][1]
                # y_ood = y_ood.reshape(-1, 1)
                tmp = get_results_mapie(coverages, mapie, X_ood, X_val, y_ood, y_train, uncertainty, key)
                res = pd.concat([res, tmp], axis=0)
...
def read_data(dataset, data_folder="data/folktables/", seed=42):
    df = pd.read_csv("{}/{}.csv".format(data_folder, dataset))
...
def get_testsets(dataset, seed, te, scaler, scalerY, perc_test, data_folder="data/folktables/"):
    dict_test = {}
    for i, el in enumerate(folk_dict[dataset]):
        X, y = read_data(el, data_folder=data_folder, seed=seed)
        X = pd.DataFrame(X.copy()).reset_index(drop=True)
        X.columns = ["Var" + str(i) for i in X.columns]
        y = pd.Series(y.copy()).values
        X_tr, X_test, y_tr, y_test = train_test_split(X, y, test_size=perc_test, random_state=seed)
        ...
        y_te = scalerY.transform(y_test.reshape(-1, 1)).flatten()
        y_te = y_te.astype(np.float32)
        dict_test[el] = (X_te.copy(), y_te.copy())
    return dict_test
...
def main(dataset, regressors, metas, nj=1, seed=42, nboots=None, device="cuda:0"):
    ...
    set_seed(seed)
    try:
        X, y = read_data(dataset, seed=seed)
    except:
    ...
    for reg_string in regressors:
        if reg_string == "lasso":
          ...
          max_iter=10000)
            ...
            reg_base = XGBRegressor(random_state=seed, n_jobs=nj, n_estimators=1)
        elif reg_string == "rf":
            reg_base = RandomForestRegressor(random_state=seed)
        elif reg_string == "mlp":
            reg_base = MLPRegressor(random_state=seed, max_iter=100, batch_size=bsize)
        elif reg_string == "mlpLarge":
            reg_base = MLPRegressor(
                random_state=seed,
                hidden_layer_sizes=(256, 256, 256, 256), batch_size=bsize, max_iter=100
            )
        elif reg_string == "ftt":
...
if __name__ == "__main__":
    list_datasets = [el.replace(".csv", "") for el in os.listdir("data/folktables")]
    ...
    for dataset in tqdm(sorted(list_datasets)[start:end]):
        print(dataset)
...
# for i,el in enumerate(nsmallest):
#     df_mean["abs_diff_{}".format(dict_mean_sorted[i][0])] = np.abs(df_mean["mean"] - el)
# for i,el in enumerate(nlargest):
#     df_mean["abs_diff_{}".format(dict_mean_sorted[-i][0])] = np.abs(df_mean["mean"] - el)


# // compute matrix with absolute distance between values
# import numpy as np
# import pandas as pd
# import matplotlib.pyplot as plt
