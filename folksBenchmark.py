...
# %%
# Train Model
for model in [Lasso(), XGBRegressor(n_jobs=4)]:
    ...
    for coverage in cov_list:
        ...
        print(
            "Doubt: Reject Rate{:.2f} samples selected {} with error {:.2f}".format(
                coverage,
...
# to dataframe
res = pd.DataFrame(
    l1table, columns=["Dataset", "Model", "Doubt", "New Doubt", "Variance"]
).T
